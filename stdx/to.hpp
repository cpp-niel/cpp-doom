...
namespace stdx
{
    ...
    namespace detail
    {
        ...
        concept recursive_container_convertible =
            container_convertible<C, R> ||(r::input_range<r::range_reference_t<R>>&& requires {
                {
    ...
    namespace detail
    {
        ...
        private:
            ...
            private:
                ...
                constexpr static auto construct(Rng&& rng, Args&&... args)
                {
                    ...
                    else if constexpr (std::constructible_from<Cont, from_range_t, Rng, Args...>)
                    {
                        ...
                        r::common_range<
                            Rng> && std::constructible_from<Cont, r::iterator_t<Rng>, r::iterator_t<Rng>, Args...>)
                    {
                        return Cont(r::begin(rng), r::end(rng), std::forward<Args>(args)...);
                    }
                    // we can do push back
                    else if constexpr (insertable_container<Cont> && std::constructible_from<Cont, Args...>)
                    {
                ...
                requires recursive_container_convertible<Cont, Rng> && std::constructible_from<Cont, Args...> &&(
                    !container_convertible<
                        Cont, Rng> && !std::constructible_from<Cont, Rng>)constexpr static auto impl(Rng&& rng,
                                                                                                     Args&&... args)
                {

                    return to<Cont, Args...>(rng | r::views::transform([](auto&& elem) {
                                                 return to<r::range_value_t<Cont>>(std::forward<decltype(elem)>(elem));
                                             }),
            ...
            public:
                template <typename Rng>
                requires r::input_range<Rng> && recursive_container_convertible<container_t<C, Rng, Args...>,
    ...
    {
        return detail::to_container_fn<detail::wrap<ContT>, Args...>{}(std::forward<Rng>(rng),
                                                                       std::forward<Args>(args)...);
    }
